...
class Peepholes:
    ...
    def get_peepholes(self, **kwargs):
        '''
        Compute labels probabilities from pred probabilities and empirical posteriors.
    ...
    def get_scores(self, **kwargs):
        ...
        for loader_name in _dls:
            ...
            if 'score_max' in _td[layer] and 'score_entropy' in _td[layer]:
                if verbose: print(f"Scores already computed for layer {layer}. Skipping score computation.")
                continue 

            #-----------------------------------------
            # Pre-allocate scores
            #-----------------------------------------
            if 'score_max' not in _td[layer]:
                if verbose: print('Allocating score_max for layer:', layer)
                _td[layer]['score_max'] = MMT.empty(shape=(n_samples,))
    
            if 'score_entropy' not in _td[layer]:
                if verbose: print('Allocating score_entropy for layer:', layer)
            ...
            for bn, batch in enumerate(tqdm(_dls[loader_name])):
                n_in = len(batch)
                peepholes = _td[layer]['peepholes'][bn*bs:bn*bs+n_in]
                _td[layer]['score_max'][bn*bs:bn*bs+n_in] = torch.max(peepholes, dim=1).values
                _td[layer]['score_entropy'][bn*bs:bn*bs+n_in] = torch.sum(peepholes * torch.log(peepholes + 1e-12), dim=1)
