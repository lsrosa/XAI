...
numpy-base                1.26.4 sh bits used by LSH.
        ...
        self.dknn_path = self.path/Path(f'{layers}/train_{pt}/val_{pv}')
        ...
        if self.dknn_path.exists():
            ...
            portion = 'train'
            bs = self._loaders[portion].batch_size
            
            print('inizio labels')
            filename = path.join(mkdtemp(), 'newfile.dat')
                
            self.train_labels = np.memmap(filename, dtype='float32', mode='w+', shape=len(self._loaders[portion].dataset))
            for bn, data in enumrate(self._loaders[portion]):
                n_in = data[1].shape[0]
                self.train_labels[bn*bs:bn*bs+n_in] = data[1]

            activations = get_activations(self._loaders[portion], model._model, self.layers)

            n_samples = len(self._loaders[portion].dataset)
            ...
            if self.percentage[portion] != 100:
                ...
                for l in np.arange(0,self.nb_classes):
                    idx_l = np.argwhere(self.train_labels==l)
